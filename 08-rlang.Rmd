...
example_data = tibble(countries = c('UK', 'USA', 'Pakistan', 'Mexico', 'Ireland', 'Estonia'),
                      ...
                      Death_from_guns = as.numeric(as.character(c('1', '200', '150', '450', '3', '3.5'))),
                      Death_from_smoking = as.numeric(as.character(c('100', '300', '140', '150', '120', '300'))))
...
summarise_feature = function(df, col_var, ...){
  require(tidyverse)
  summary_var <- enquo(col_var)
  summary_nm <- quo_name(summary_var)
  summary_nm_sum <- paste0("metric_",summary_nm)#The new LHS variable must have a different name from that on the RHS
  
  df %>%
    group_by(...) %>% 
    summarise(!!summary_nm_sum := sum(!!summary_var))}

#output new variable
example_data %>% 
  summarise_feature(Death_from_guns, region) #Metric can be provided by a shiny input too i.e. UQ(rlang::sym(input$metric))
