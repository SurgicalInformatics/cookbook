...
\NormalTok{mytable =}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x, }\DataTypeTok{caption =} \StringTok{""}\NormalTok{, }\DataTypeTok{longtable =} \OtherTok{FALSE}\NormalTok{, ...)\{}
  
  \CommentTok{# if not latex or html then else is Word}
  \ControlFlowTok{if}\NormalTok{ (knitr}\OperatorTok{::}\KeywordTok{is_latex_output}\NormalTok{() }\OperatorTok{|}\StringTok{ }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{is_html_output}\NormalTok{())\{}
\NormalTok{    knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(x, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{align =} \KeywordTok{c}\NormalTok{(}\StringTok{"l"}\NormalTok{, }\StringTok{"l"}\NormalTok{, }\StringTok{"r"}\NormalTok{, }\StringTok{"r"}\NormalTok{, }\StringTok{"r"}\NormalTok{, }\StringTok{"r"}\NormalTok{, }\StringTok{"r"}\NormalTok{, }\StringTok{"r"}\NormalTok{, }\StringTok{"r"}\NormalTok{), }
          \DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{caption =}\NormalTok{ caption, }\DataTypeTok{longtable =}\NormalTok{ longtable,}
          \DataTypeTok{linesep =} \StringTok{""}\NormalTok{, ...) }\OperatorTok{%>%}
\StringTok{    }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \KeywordTok{c}\NormalTok{(}\StringTok{"scale_down"}\NormalTok{, }\StringTok{"hold_position"}\NormalTok{))}
\NormalTok{  \}}\ControlFlowTok{else}\NormalTok{\{}
\NormalTok{    flextable}\OperatorTok{::}\KeywordTok{flextable}\NormalTok{(x) }\OperatorTok{%>%}\StringTok{ }
\StringTok{      }\NormalTok{flextable}\OperatorTok{::}\KeywordTok{autofit}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }
\StringTok{      }\NormalTok{flextable}\OperatorTok{::}\KeywordTok{width}\NormalTok{(}\DataTypeTok{j =} \DecValTok{1}\NormalTok{, }\DataTypeTok{width =} \FloatTok{1.5}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{      }\NormalTok{flextable}\OperatorTok{::}\KeywordTok{height}\NormalTok{(}\DataTypeTok{i =} \DecValTok{1}\NormalTok{, }\DataTypeTok{height =} \FloatTok{0.5}\NormalTok{, }\DataTypeTok{part =} \StringTok{"header"}\NormalTok{)}
\NormalTok{  \}}
...
\StringTok{  }\KeywordTok{mytable}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{how-to-contribute-1}{%
\section{How to contribute}\label{how-to-contribute-1}}
...
\hypertarget{chapter-and-section-references-1}{%
\subsection{Chapter and section references}\label{chapter-and-section-references-1}}

You can label chapter and section titles using \texttt{\{\#label\}} after them, e.g., we can reference Chapter \texttt{\textbackslash{}@ref(intro)} (ticks in .Rmd are excluded when actually using). If you do not manually label them, there will be automatic labels anyway, e.g., Chapter \texttt{\textbackslash{}@ref(methods)}.

\hypertarget{figure-and-table-references-1}{%
\subsection{Figure and table references}\label{figure-and-table-references-1}}
...
\hypertarget{snippets-1}{%
\chapter{Snippets}\label{snippets-1}}

Random useful snippets that do not fit anywhere else.

\hypertarget{exporting-tables-that-work-in-both-pdf-and-word-1}{%
\section{Exporting tables that work in both PDF and Word}\label{exporting-tables-that-work-in-both-pdf-and-word-1}}
...
\hypertarget{creating-reproducible-r-examples-to-share-in-the-group-binder-holepunch-and-docker-1}{%
\section{Creating Reproducible R Examples to Share in the Group (binder, holepunch and docker)}\label{creating-reproducible-r-examples-to-share-in-the-group-binder-holepunch-and-docker-1}}

When asking for help with R code having a reproducible example is crucial (some mock data that others can use along with your code to reproduce your error). Often this can be done easily with creation of a small tibble and posting of the code on slack but sometimes it requires more complex data or the error is due to something in the Linux system in which RStudio server is hosted. For example if the \texttt{cairo} package for Linux isn't installed then plots don't work. The \texttt{holepunch} package helps to reproduce examples like these (not suitable for projects with confidential data).

\hypertarget{create-basic-reproducible-examples-1}{%
\subsection{Create Basic Reproducible Examples}\label{create-basic-reproducible-examples-1}}
...
\hypertarget{holepunch---complex-reproducible-examples-1}{%
\subsection{\texorpdfstring{\texttt{holepunch} - Complex Reproducible Examples}{holepunch - Complex Reproducible Examples}}\label{holepunch---complex-reproducible-examples-1}}
...
\hypertarget{working-with-chis-1}{%
\section{Working with CHIs}\label{working-with-chis-1}}

Here are 4 functions for CHIs that could even be put in a small package.
The Community Health Index (CHI) is a population register, which is used in Scotland for health care purposes.
The CHI number uniquely identifies a person on the index.

\hypertarget{chi_dob---extract-date-of-birth-from-chi-1}{%
\subsection{\texorpdfstring{\texttt{chi\_dob()} - Extract date of birth from CHI}{chi\_dob() - Extract date of birth from CHI}}\label{chi_dob---extract-date-of-birth-from-chi-1}}
...
\hypertarget{chi_gender---extract-gender-from-chi-1}{%
\subsection{\texorpdfstring{\texttt{chi\_gender()} - Extract gender from CHI}{chi\_gender() - Extract gender from CHI}}\label{chi_gender---extract-gender-from-chi-1}}
...
\hypertarget{chi_age---extract-age-from-chi-1}{%
\subsection{\texorpdfstring{\texttt{chi\_age()} - Extract age from CHI}{chi\_age() - Extract age from CHI}}\label{chi_age---extract-age-from-chi-1}}
...
\hypertarget{chi_valid---logical-test-for-valid-chi-1}{%
\subsection{\texorpdfstring{\texttt{chi\_valid()} - Logical test for valid CHI}{chi\_valid() - Logical test for valid CHI}}\label{chi_valid---logical-test-for-valid-chi-1}}
...
\hypertarget{working-with-dates-1}{%
\section{Working with dates}\label{working-with-dates-1}}

\hypertarget{difference-between-two-dates-1}{%
\subsection{Difference between two dates}\label{difference-between-two-dates-1}}
...
\hypertarget{pulling-out-change-in-status-data-1}{%
\subsection{Pulling out ``change in status'' data}\label{pulling-out-change-in-status-data-1}}
...
\hypertarget{compute-time-from-op-date-to-current-review-1}{%
\subsubsection{Compute time from op date to current review}\label{compute-time-from-op-date-to-current-review-1}}
...
\hypertarget{pull-out-change-of-status-1}{%
\subsubsection{Pull out ``change of status''}\label{pull-out-change-of-status-1}}
...
\hypertarget{run-cph-1}{%
\subsubsection{Run CPH}\label{run-cph-1}}
...
\hypertarget{collapse-multiple-no-and-yes-options-1}{%
\section{Collapse multiple ``no'' and ``yes'' options}\label{collapse-multiple-no-and-yes-options-1}}
...
\hypertarget{filter-na-dropping-rows-where-all-specified-variables-are-na-1}{%
\section{Filter NA: Dropping rows where all specified variables are NA}\label{filter-na-dropping-rows-where-all-specified-variables-are-na-1}}
...
\hypertarget{pulling-images-from-redcap-directly-to-argodeep-1}{%
\subsection{Pulling images from REDCap directly to argodeep}\label{pulling-images-from-redcap-directly-to-argodeep-1}}

\hypertarget{original-file-names-1}{%
\subsubsection{Original file names}\label{original-file-names-1}}
...
\hypertarget{named-from-redcap-record-id-and-event-1}{%
\subsubsection{Named from REDCap record ID and event}\label{named-from-redcap-record-id-and-event-1}}
...
\hypertarget{data-transfer-and-eddie-1}{%
\chapter{Data Transfer and Eddie}\label{data-transfer-and-eddie-1}}

Often it is necessary to transfer data into and out of RStudio server. This can be done from personal laptops (as long as you have permission for the data!), university supported desktops, Eddie and a number of other devices or servers.

\hypertarget{uploading-and-downloading-data-the-easy-way-1}{%
\section{Uploading and Downloading Data the Easy Way}\label{uploading-and-downloading-data-the-easy-way-1}}

Often the GUI in RStudio is sufficient. In the \texttt{Files} pane click upload to move data from your current computer into RStudio server. To download select the file and then click \texttt{More\ \textgreater{}\ Export}. As always this should be done only when appropriate.

Sometimes this isn't possible either with large files or files stored remotely on other servers such as Eddie.

\hypertarget{alternative-methods-when-the-easy-way-wont-work-1}{%
\section{Alternative Methods when the Easy Way won't work}\label{alternative-methods-when-the-easy-way-wont-work-1}}

\hypertarget{what-is-eddie-1}{%
\subsection{What is Eddie?}\label{what-is-eddie-1}}

Eddie Mark 3 is the third iteration of the University's compute cluster and is available to all University of Edinburgh researchers. It consists of some 7000 Intel® Xeon® cores with up to 3 TB of memory available per compute node.

The surgical informatics group has a shared folder in the Eddie cluster which can be accessed to store data and perform analyses which might be too large or complex for RStudio server. Every task in Eddie is controlled through the command line interface - those familiar with Linux/Unix will be familiar with this.

\hypertarget{using-the-command-line-1}{%
\section{Using the Command Line}\label{using-the-command-line-1}}

Sometimes the command line is the only possible way to copy data to and from RStudio server. Argonaut and Argosafe are \texttt{SSH}-enabled (Secure SHell) meaning that you can securely copy data to and from them using the command line editor on another device. It is also possible to use the RStudio terminal to copy to another device or server that is \texttt{SSH}-enabled although this isn't currently recommended due to issues with RStudio server's websockets (when you type in the terminal you have to do so very slowly for characters to appear in the right order or you have to use a script - described below).

First, if you don't have a command line editor or SSH client installed (often the case for earlier Windows versions although there is talk of a native client becoming default) then you will need to install one. For working on a Windows device generally PuTTY is the recommended SSH client (allows you connect to other servers) and a reasonable command line editor to work with files on the local device is GitBash.

\hypertarget{downloads-and-setup---eddie-example-1}{%
\subsection{Downloads and Setup - Eddie Example}\label{downloads-and-setup---eddie-example-1}}
...
\hypertarget{copy-and-paste-with-putty-1}{%
\subsection{Copy and Paste with PuTTY}\label{copy-and-paste-with-putty-1}}
...
\hypertarget{closing-a-session-in-putty-1}{%
\subsection{Closing a Session in PuTTY}\label{closing-a-session-in-putty-1}}

To close a session use \texttt{ctrl-d}.

\hypertarget{eddie-file-structure-1}{%
\subsection{Eddie File Structure}\label{eddie-file-structure-1}}
...
\hypertarget{copying-data-from-eddie-into-rstudio-server-1}{%
\section{Copying Data from Eddie into RStudio server}\label{copying-data-from-eddie-into-rstudio-server-1}}

\hypertarget{method-1-using-putty-or-another-terminalshell-connected-to-eddie-1}{%
\subsection{Method 1: Using PuTTY (or another terminal/shell connected to Eddie)}\label{method-1-using-putty-or-another-terminalshell-connected-to-eddie-1}}
...
\hypertarget{method-2-using-the-rstudio-server-terminal-1}{%
\subsection{Method 2: Using the RStudio Server Terminal}\label{method-2-using-the-rstudio-server-terminal-1}}
...
\hypertarget{the-surgical-informatics-group-folder-on-eddie-1}{%
\section{The Surgical Informatics Group Folder (On Eddie)}\label{the-surgical-informatics-group-folder-on-eddie-1}}
...
\hypertarget{the-scratch-space-1}{%
\subsection{The Scratch Space}\label{the-scratch-space-1}}
...
\hypertarget{other-spaces-1}{%
\subsection{Other Spaces}\label{other-spaces-1}}

Occasionally group members may have access to other directories to share / collaborate on projects. These are most often found at the path: \texttt{cd\ /exports/\textless{}COLLEGE\textgreater{}/eddie/\textless{}SCHOOL\textgreater{}/groups/\textless{}GROUP\textgreater{}} although for IGMM it may be: \texttt{cd\ /exports/igmm/eddie/\textless{}GROUP\textgreater{}}.

\hypertarget{running-eddie-from-rstudio-server-1}{%
\subsection{Running Eddie from RStudio Server}\label{running-eddie-from-rstudio-server-1}}
...
\hypertarget{modules-applications-in-eddie-1}{%
\subsection{Modules (Applications) in Eddie}\label{modules-applications-in-eddie-1}}
...
\hypertarget{working-with-r-in-eddie-1}{%
\subsection{Working with R in Eddie}\label{working-with-r-in-eddie-1}}

\hypertarget{rprofile-file-1}{%
\subsubsection{.Rprofile File}\label{rprofile-file-1}}
...
\hypertarget{creating-an-.rprofile-file-1}{%
\subsubsection{Creating an .Rprofile File}\label{creating-an-.rprofile-file-1}}
...
\hypertarget{renviron-file-1}{%
\subsubsection{.Renviron File}\label{renviron-file-1}}
...
\hypertarget{specific-r-package-versions-1}{%
\subsection{Specific R Package Versions}\label{specific-r-package-versions-1}}
...
\hypertarget{general-eddie-etiquette-1}{%
\subsection{General Eddie Etiquette}\label{general-eddie-etiquette-1}}

Don't overwrite or change other users files without permission and set up your own files to prevent others doing that if appropriate. Please don't leave configuration files like \texttt{.Renviron} files in the shared space as this can play havoc with other users R tasks in Eddie - if you need a particular configuration (e.g.~a default package library for a project) then best to leave the config files in the directory for that project (or in your own home directory).

\hypertarget{unix-tutorials-1}{%
\section{Unix Tutorials}\label{unix-tutorials-1}}
...
\hypertarget{gghighlight-example-1}{%
\section{GGHighlight Example}\label{gghighlight-example-1}}
...
\hypertarget{coord_flip-factor-orders-1}{%
\section{\texorpdfstring{\texttt{coord\_flip()} factor orders}{coord\_flip() factor orders}}\label{coord_flip-factor-orders-1}}
...
\includegraphics{cookbook_files/figure-latex/unnamed-chunk-45-2.pdf}
...
\includegraphics{cookbook_files/figure-latex/unnamed-chunk-45-3.pdf}

\hypertarget{axis-font-size-1}{%
\section{Axis font size}\label{axis-font-size-1}}
...
\includegraphics{cookbook_files/figure-latex/unnamed-chunk-46-1.pdf}
...
\hypertarget{single-cell-analysis-1}{%
\section{Single Cell Analysis}\label{single-cell-analysis-1}}

\hypertarget{minimising-the-size-of-a-seurat-object-1}{%
\subsection{Minimising the size of a Seurat Object}\label{minimising-the-size-of-a-seurat-object-1}}
...
\hypertarget{programming-in-rlang-1}{%
\chapter{Programming in rlang}\label{programming-in-rlang-1}}

\hypertarget{rlang-1}{%
\section{rlang}\label{rlang-1}}

\hypertarget{what-is-rlang-1}{%
\subsection{What is rlang?}\label{what-is-rlang-1}}

rlang is a low-level programming API for R which the tidyverse uses (meaning it speaks to R in as R like way as possible, rather than a `high-level' - high level is more user orientated and interpretable). It enables you to extend what the tidyverse can do and adapt it for your own uses. It's particularly good to use if you're doing lots of more `programming' type R work, for example, building a package, making a complex shiny app or writing functions. It might also be handy if you're doing lots of big data manipulation and want to manipulate different datasets in the same way, for example.

In this chapter we'll discuss some uses of it.

\hypertarget{dynamic-calling-of-variables-using-dplyr-1}{%
\subsection{Dynamic calling of variables using dplyr}\label{dynamic-calling-of-variables-using-dplyr-1}}
...
\hypertarget{server-admin-1}{%
\chapter{Server admin}\label{server-admin-1}}

\hypertarget{rstudio-connect-argoshare-1}{%
\section{RStudio Connect: argoshare}\label{rstudio-connect-argoshare-1}}

\hypertarget{delete-user-1}{%
\subsection{Delete user}\label{delete-user-1}}
...
\hypertarget{citations-with-r-markdown-1}{%
\chapter{Citations with R Markdown}\label{citations-with-r-markdown-1}}
...
\hypertarget{zotero-set-up-1}{%
\section{Zotero Set-up}\label{zotero-set-up-1}}
...
\hypertarget{file-storage-set-up-1}{%
\section{File Storage Set-up}\label{file-storage-set-up-1}}

In addition to Zotero set-up it is a good idea to have a cloud storage service to maintain a library of PDFs without using up Zotero storage capacity. These will also need to be accessible to the \texttt{Windows\ Explorer} or the \texttt{Mac\ Finder} - there are several guides on how to do this for \href{https://www.howtogeek.com/228989/how-to-use-the-desktop-google-drive-app/}{Google Drive} and \href{https://www.dropboxforum.com/t5/Installation-and-desktop-app/How-do-I-put-Dropbox-on-my-File-Explorer-in-Win-10/td-p/313031}{DropBox}.

\hypertarget{folder-set-up-1}{%
\section{Folder Set-up}\label{folder-set-up-1}}

\hypertarget{storing-pdfs-1}{%
\subsection{Storing PDFs}\label{storing-pdfs-1}}

Create a folder in a cloud storage service to store PDFs which can be accessed from your own file explorer.

\hypertarget{storing-bibliographies-1}{%
\subsection{Storing Bibliographies}\label{storing-bibliographies-1}}

Create a folder which will store your \texttt{.bib} file. This can then be used to provide the references for your \texttt{.rmd} files.

\hypertarget{subfolder-for-references-1}{%
\subsection{Subfolder for References}\label{subfolder-for-references-1}}

It may be a good idea in Zotero to carefully categorise publications by their content. This makes it very easy to look up publications on a given topic (Zotero allows a citation to be shared across several folders such that a publication documenting an RCT with updated meta-analysis could be in folders for each type of study etc.).

Generally a parent folder will be used to generate a bibliography file and anything likely to be cited in a document / thesis should be in that folder or one of its subfolders.

\hypertarget{configuring-zotero-1}{%
\section{Configuring Zotero}\label{configuring-zotero-1}}

\hypertarget{zotfile-pdf-preferences-1}{%
\subsection{ZotFile PDF Preferences}\label{zotfile-pdf-preferences-1}}
...
\hypertarget{general-zotero-settings-1}{%
\subsection{General Zotero Settings}\label{general-zotero-settings-1}}
...
\hypertarget{refresh-bibtex-citation-key-1}{%
\subsection{Refresh BibTex Citation Key}\label{refresh-bibtex-citation-key-1}}

If you already have used Zotero without this setup and want to refresh your citation keys to follow the standard pattern then select all references, right click and use ``Better BibTex → Refresh BibTeX Key''.

\hypertarget{generating-a-.bib-file-1}{%
\section{Generating a .bib File}\label{generating-a-.bib-file-1}}
...
\hypertarget{linking-dropbox-and-rstudio-server-1}{%
\section{Linking DropBox and Rstudio Server}\label{linking-dropbox-and-rstudio-server-1}}

Dropbox provides a token to allow communication between different apps. The \texttt{rdrop2} package allows this. It may be necessary to create the token on RStudio desktop as creation on the server is buggy but this is perfectly ok.

\textbf{Caution:} The token generated by this process could be used to access your Dropbox from anywhere using RStudio if you do not keep it secure. If somebody were to access an unencrypted token then it would be equivalent to handing out your email and password. Use the \texttt{encryptr} package to allow safe storage of this token.

\hypertarget{token-creation-1}{%
\subsection{Token Creation}\label{token-creation-1}}
...
\hypertarget{dropbox-linkage-1}{%
\subsection{DropBox Linkage}\label{dropbox-linkage-1}}
...
\hypertarget{citing-r-packages-1}{%
\section{Citing R Packages}\label{citing-r-packages-1}}
