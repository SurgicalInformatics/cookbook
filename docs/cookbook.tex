...
\includegraphics[width=30.97in]{/home/common/users/cfairfield/cookbook/img/surgical_informatics_minibanner}
...
\NormalTok{tdrake_packages =}\StringTok{ }\KeywordTok{installed.packages}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }\CommentTok{#change 'tdrake' to your rstudio username}
\StringTok{  }\KeywordTok{as_tibble}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(LibPath }\OperatorTok{%>%}\StringTok{ }\KeywordTok{str_detect}\NormalTok{(}\StringTok{"tdrake"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{pull}\NormalTok{(Package)}

\NormalTok{system_packages =}\StringTok{ }\KeywordTok{installed.packages}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{as_tibble}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(LibPath }\OperatorTok{%>%}\StringTok{ }\KeywordTok{str_detect}\NormalTok{(}\StringTok{"local"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{pull}\NormalTok{(Package)}

\NormalTok{to_remove =}\StringTok{ }\NormalTok{tdrake_packages[tdrake_packages }\OperatorTok{%in%}\StringTok{ }\NormalTok{system_packages]}

\KeywordTok{remove.packages}\NormalTok{(to_remove)}
\ErrorTok{\}}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mydata }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{count}\NormalTok{(ssi.factor)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mydata }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\KeywordTok{across}\NormalTok{(}\KeywordTok{where}\NormalTok{(is.yn), fct_collapse_yn, }\DataTypeTok{.names =} \StringTok{"\{col\}_yn"}\NormalTok{)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{count}\NormalTok{(ssi.factor_yn)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=0.5\linewidth]{/home/common/users/cfairfield/cookbook/img/barplot_5x4}
\includegraphics[width=0.5\linewidth]{/home/common/users/cfairfield/cookbook/img/barplot_10x8}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{DAG <-}\StringTok{ }\NormalTok{ggdag}\OperatorTok{::}\KeywordTok{dagify}\NormalTok{(y }\OperatorTok{~}\StringTok{ }\NormalTok{x)}
\NormalTok{DAG <-}\StringTok{ }\NormalTok{ggdag}\OperatorTok{::}\KeywordTok{tidy_dagitty}\NormalTok{(DAG)}
\NormalTok{PLOT1 <-}\StringTok{ }\NormalTok{ggdag}\OperatorTok{::}\KeywordTok{ggdag}\NormalTok{(DAG, }\DataTypeTok{node_size =} \DecValTok{50}\NormalTok{)}
\NormalTok{PLOT1}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{shorten_dag_arrows <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(tidy_dag, shorten_distance)\{}
  \CommentTok{# Update underlying ggdag object}
\NormalTok{  tidy_dag}\OperatorTok{$}\NormalTok{data =}\StringTok{ }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(tidy_dag}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{slope =}\NormalTok{ (yend }\OperatorTok{-}\StringTok{ }\NormalTok{y) }\OperatorTok{/}\StringTok{ }\NormalTok{(xend }\OperatorTok{-}\StringTok{ }\NormalTok{x), }\CommentTok{# Calculate slope of line}
                  \DataTypeTok{distance =} \KeywordTok{sqrt}\NormalTok{((xend}\OperatorTok{-}\NormalTok{x)}\OperatorTok{^}\DecValTok{2} \OperatorTok{+}\StringTok{ }\NormalTok{(yend }\OperatorTok{-}\StringTok{ }\NormalTok{y)}\OperatorTok{^}\DecValTok{2}\NormalTok{), }\CommentTok{# Calculate total distance of line}
                  \DataTypeTok{proportion =}\NormalTok{ shorten_distance}\OperatorTok{/}\NormalTok{distance, }\CommentTok{# Calculate proportion by which to be shortened}
                  \DataTypeTok{xend =}\NormalTok{ (}\DecValTok{1}\OperatorTok{-}\NormalTok{proportion)}\OperatorTok{*}\NormalTok{xend }\OperatorTok{+}\StringTok{ }\NormalTok{(proportion}\OperatorTok{*}\NormalTok{x), }\CommentTok{# Shorten xend}
                  \DataTypeTok{yend =}\NormalTok{ (}\DecValTok{1}\OperatorTok{-}\NormalTok{proportion)}\OperatorTok{*}\NormalTok{yend }\OperatorTok{+}\StringTok{ }\NormalTok{(proportion}\OperatorTok{*}\NormalTok{y)) }\OperatorTok{%>%}\StringTok{ }\CommentTok{# Shorten yend}
\StringTok{    }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{select}\NormalTok{(}\OperatorTok{-}\NormalTok{slope, }\OperatorTok{-}\NormalTok{distance, }\OperatorTok{-}\NormalTok{proportion) }\CommentTok{# Drop intermediate values}
  \KeywordTok{return}\NormalTok{(tidy_dag)}
\NormalTok{\}}
\NormalTok{DAG <-}\StringTok{ }\KeywordTok{shorten_dag_arrows}\NormalTok{(DAG, }\DataTypeTok{shorten_distance =} \FloatTok{0.1}\NormalTok{)}
\NormalTok{PLOT2 <-}\StringTok{ }\NormalTok{ggdag}\OperatorTok{::}\KeywordTok{ggdag}\NormalTok{(DAG, }\DataTypeTok{node_size =} \DecValTok{50}\NormalTok{)}
\NormalTok{PLOT2}
\end{Highlighting}
\end{Shaded}
...
\StringTok{    }\KeywordTok{summarise}\NormalTok{(}\OperatorTok{!!}\DataTypeTok{summary_nm_sum :=} \KeywordTok{sum}\NormalTok{(\{\{col_var\}\}))}
\NormalTok{\}}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\OperatorTok{!!}\DataTypeTok{metric_mutate :=} \OperatorTok{!!}\NormalTok{rlang}\OperatorTok{::}\KeywordTok{sym}\NormalTok{(metric) }\OperatorTok{*}\StringTok{ }\DecValTok{2}\NormalTok{)}\CommentTok{#apply arbitary multiplication by two}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{server}\NormalTok{ \{}
    ...
    \ExtensionTok{location}\NormalTok{ / \{}
        \ExtensionTok{proxy_pass}\NormalTok{ http://127.0.0.1:3838/}\KeywordTok{;}
        \ExtensionTok{proxy_redirect}\NormalTok{ http://127.0.0.1:3838/ }\VariableTok{$scheme}\NormalTok{://}\VariableTok{$host}\NormalTok{/}\KeywordTok{;}
        \CommentTok{# note, the following 3 lines added 2016-07-11, see updates section}
        \ExtensionTok{proxy_http_version}\NormalTok{ 1.1}\KeywordTok{;}
        \ExtensionTok{proxy_set_header}\NormalTok{ Upgrade }\VariableTok{$http_upgrade}\KeywordTok{;}
        \ExtensionTok{proxy_set_header}\NormalTok{ Connection }\StringTok{"upgrade"}\KeywordTok{;}
\NormalTok{     \}}
\NormalTok{    \}}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{location}\NormalTok{ / \{}
      \ExtensionTok{proxy_pass}\NormalTok{ http://127.0.0.1:3838/}\KeywordTok{;}
      \ExtensionTok{proxy_redirect}\NormalTok{ http://127.0.0.1:3838/ }\VariableTok{$scheme}\NormalTok{://}\VariableTok{$host}\NormalTok{/}\KeywordTok{;}
      \ExtensionTok{auth_basic} \StringTok{"Username and Password are required"}\KeywordTok{;} 
      \ExtensionTok{auth_basic_user_file}\NormalTok{ /etc/nginx/.htpasswd}\KeywordTok{;}
\NormalTok{    \}}
\end{Highlighting}
\end{Shaded}
