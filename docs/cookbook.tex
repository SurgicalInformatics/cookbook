...
\item
  In the terminal, do
  \texttt{git\ clone\ git@github.com:SurgicalInformatics/cookbook.git}.
\item
...
\item
  ...
  each chapter. Use the Build tab to Build your changes into a book.
\item
  If anyone has pushed since you cloned/last pulled (hopefully they've
  not been working on the exact same chapter): click on More - Clean
  All. Then Pull from the Git tab. This only cleans the output files -
  html and PDF, it will not touch the changes you've made in the .Rmd
  file.
\item
...
\hypertarget{extract-dob-and-gender-from-chi}{%
\section{Extract dob and gender from chi}\label{extract-dob-and-gender-from-chi}}
...
\NormalTok{x =}\StringTok{ }\KeywordTok{tibble}\NormalTok{(}
  \DataTypeTok{chi =} \KeywordTok{c}\NormalTok{(}\StringTok{"1009761234"}\NormalTok{, }\StringTok{"1009762345"}\NormalTok{)}
\NormalTok{  )}

\NormalTok{x }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
    \DataTypeTok{dob =} \KeywordTok{str_extract}\NormalTok{(chi, }\StringTok{".\{6\}"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{      }\KeywordTok{dmy}\NormalTok{(),}
    \DataTypeTok{gender =} \KeywordTok{str_sub}\NormalTok{(chi, }\DecValTok{9}\NormalTok{, }\DecValTok{9}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
##   chi        dob        gender
##   <chr>      <date>     <chr> 
## 1 1009761234 1976-09-10 Male  
## 2 1009762345 1976-09-10 Female
\end{verbatim}
